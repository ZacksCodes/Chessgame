...
class user{
		...
		if($UserRequest->rowCount()==0){
			...
			while($row=$UserRequest->fetch()){
				...
				if ($row["GameId"] == 0) {
					...
					header("Location: ../chMult/pages/Ch/index.php");
				}  
				return true;
			}
		} 
	...
	public function DisplayAvailablePlayers(){
		include "connectToDB.php";
		$UserReq=$_db->query("SELECT * FROM users where GameId=0  ORDER BY RAND() limit 4 ");
			...
			while($row = $UserReq->fetch(PDO::FETCH_ASSOC)){
				...
				if($row["UserName"] != $_SESSION['UserName']) { // if opponent name
					...
					if ($row["GameId"] != 0) {
						...
						?>
						<span class="UserNameS" onclick="parent.location='php_pages/redirect.php?id=<?php echo $token;?>&name=<?php echo $name;?>';"> <?php echo $name;?>
						</span>
						<?php
